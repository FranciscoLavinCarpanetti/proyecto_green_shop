...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Simulación de productos (36 productos para 3 páginas)
  const productos = Array.from({ length: 36 }, (_, i) => ({
    id: i + 1,
    nombre: `Producto ${i + 1}`,
    descripcion: `Descripción del producto ${i + 1}`,
    precio: (Math.random() * 10 + 1).toFixed(2),
    categoria: i % 2 === 0 ? "Flores" : "Plantas",
  ...
  // Función para cambiar de página
  const cambiarPagina = (direccion) => {
    const productosFiltrados = tipoPlantaSelect.value === "Todas"
      ? productos
      : productos.filter(producto =>
          producto.categoria.toLowerCase().includes(tipoPlantaSelect.value.toLowerCase())
        );

    const totalPaginas = Math.ceil(productosFiltrados.length / productosPorPagina);

    if (direccion === "anterior" && paginaActual > 1) {
      paginaActual--; // Retroceder una página
    } else if (direccion === "posterior" && paginaActual < totalPaginas) {
      paginaActual++; // Avanzar una página
    }

    mostrarProductos(tipoPlantaSelect.value); // Mostrar los productos de la nueva página
  };
